...
thMockServerModule.factory('serverListsService', function(configService) {
  ...
  */
  ...
  o.twitterUsernames = ['kirkouimet','damenleeturks','calebogden','aaronbushnell','rogie','jacobseethaler','daryl','kolage','VinThomas','ShaunMoynihan','zulsdesign','nckjrvs','timothycd','motherfuton','jayrobinson','cameronmoll','jayman','danielhaim','alagoon','andrewpautler','garrettgee','blakesimkins','gilbertglee','ogvidius','manspaugh','ripplemdk','Anotherdagou','ryanleroux','roybarberuk','russellsmith21','PtiteNoli','walterstephanie','imfine_thankyou','utroda','NastyaVZ','tiagocamargo','mikebeecham','nimaa','sajtoo','AngelZxxWingZ','OskarLevinson','feliperibeiros','jonsuh','leevigraham','ryanAmurphy','jsngr','axelbouaziz','kristijantaken','decarola','iamlouisbullock','dbox','molovo','Djeje','antoniopratas','matejsudar','mkalalang','WhatTheFerguson','SiskaFlaurensia','deimler','benhowdle','mds','itolmach','MarkusOkur','ckor','bradenhamm','gabediaz','ThisIsJohnBrown','benpeck','pizzulata','haibnu','JuliaYunLiu','vctrfrnndz','redkeg','benefritz','owlfurty','eldelentes','mambows','max9xs','waqar_alamgir','CrafterSama','calvintennant','smharley','sodevious','razvantugui','FreelanceNathan','renbyrd','adn','jjmpsp','Fitehal','meghanglass','cibawoman','iamlouisbullock','desaiguddu','brianmaloney','HugoAlbonete','macvhustle','rizwaniqbal','fabioChannel','vehbikilic','kkbethi','poopsplat','wrightmartin','JeffChausse','faridelnasire','devstrong','odaymashalla','Rafa3mil','meddeg','brampitoyo','arjunchetna','toodlenoodle','iamjamie','jcarlosweb','temonehm','gerwitz','neweravin','hvillega','mozato','mcmieras','zametniy','jwphillips','Fubaruba','luhman','Betraydan','dvidsilva'];
  ...
  var teacherStatuses = [{id:0,name:'Joined'},{id:2,name:'80% complete'},{id:3,name:'Pending approval'},{id:1,name:'Approved'},{id:"-1",name:'Declined'}];
...
thMockServerModule.run(function($httpBackend, $resource, $q, $timeout, $http,
  serverListsService, randomDataService, configService, serverUtilsService) {
  ...
  json.filenames = [ 'lists', 'school101', 'school1234', 'city146', 'city25',
    'spep-structure', 'spep-notes-structure', 'content-item-showcase-structure', 'admin-school-structure',
    'jobs-app-lists', 'jobs',
    'development',
    'candidates', 'candidates-app-lists', 
  ];

  _.each(json.filenames, function(filename) {
    $http
      .get('/th-frontend/scaffolding/server/json/' + filename + '.json')
      .then(function(response) { json[serverUtilsService.camelCase(filename)] = response.data; });
  });
  ...
  var teachersResponse = function(method, url, data, headers) {
    var teachers;
    var params = serverUtilsService.deserializeParams(data); //note: all params are now strings
  ...
  var jobsResponse = function(method, url, data, headers) {
    //params
    var params = serverUtilsService.deserializeParams(data);
  ...
  //candidates
  var candidateProperties = ['id', 'dateCreated', 'subject', 'role', 'schoolName', 'country', 'numApplied', 'numPutForward', 'numShortlisted', 'numInterviewed', 'numOffersMade', 'isAccepted', 'numRejected'];

  var candidatesAppCandidatesResponse = function(params) {
    var numberOfFilters = _.size(params)-1;
    var o = _.clone(json.candidates, true);
    ...
    for (var i = 0; i < numberOfRecords/json.candidates.candidates.length; i++) { //create at least enough dummy data
      o.candidates = o.candidates.concat(_.clone(json.candidates.candidates, true));
    }
    o.candidates.length = numberOfRecords; //remove dummy data which isn't needed

    o.type = _.random(1, 3);
    _.forEach(o.candidates, function(candidate) {
      if (o.type !== 1) delete candidate.url;
      if (params.subject) candidate.subject = _.parseInt(params.subject);
      if (params.position) candidate.position = _.parseInt(params.position);
    });

    o.candidates = _.shuffle(o.candidates);
    return o;
  };

  var candidatesResponse = function(method, url, data, headers) {
    //params
    var params = serverUtilsService.deserializeParams(data);
    if (params.app === 'candidates') return [200, candidatesAppCandidatesResponse(params)];
    ...
    var candidates = {
      "candidates": randomDataService.getRandomArrayOfObjects({ properties: properties, length: length })
    };
  ...
  //candidates
    $httpBackend.whenPOST('/admin/service/candidates').respond(candidatesResponse); //returns something different each time
  //applications
  ...
  //lists
    ...
    $httpBackend.whenPOST('/candidates/service/lists').respond(function() { return [200, json.candidatesAppLists]; });
  //schools
  ...
  //meta
    $httpBackend.whenPOST('development').respond(function() { return [200, json.development]; });
...
thMockServerModule.factory('serverUtilsService', function() {
  var o = {};
  ...
  o.deserializeParams = function(p) { //see https://github.com/pythondave/th-admin/issues/11
