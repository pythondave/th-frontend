...
thGenericModule.factory('listService', function($timeout) {
  ...
  listService.List = function(arr, type) {
    ...
    o.add = function(item, role) {
      //role: false=>add to start; true=>add to end; number=>add to that position
      if (typeof role === 'boolean') { role = (role ? o.data.length : 0); }
      o.latestId++;
      item.id = o.latestId;
      o.data.splice(role, 0, item);
      return o;
    };
    ...
    o.addAndRemoveAfterDelay = function(item, role, ms) {
      o.add(item, role);
    ...
    o.sort = function(path) {
      ...
      if (path === sortOrderPaths[0]) path = _.invertString(path); //need to invert
